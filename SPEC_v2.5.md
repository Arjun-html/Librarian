...
**Spec version:** 2.5 — covers are now cached locally to survive Open Library outages (new `cache-covers` command + a best-effort auto-cache pass folded into `generate`, `--no-cache` to skip); the lead hero story's blurb is now derived from the book's own notes (first 100 chars of its per-book-page text) instead of the `hero_body` field; `render_book_card()` dead code confirmed **deleted**; §5/§6/§13/§18 updated accordingly.
...
### 🗑️ Dead code
- **`render_book_card()`** — **deleted** (v2.4 housekeeping). The per-section card grid it served was removed in v2.2; the function is now gone from `librarian.py` entirely.
- **`ai_notes` field** — still in `library.db` and still populated, but no longer displayed anywhere on the site (the front-page card grid that showed it was removed in v2.2; per-book pages never showed it). **Decision: remove from display permanently.** The field stays in the DB as a reference without being rendered.
...
### ✅ v2.5 work items — all shipped (June 2026)

1. **Local cover caching (offline-proof covers)** — **Done.** Covers used to load from `covers.openlibrary.org` by ISBN at view time, so an Open Library outage blanked every ISBN-based cover to the "No cover available" fallback. New **`cache-covers`** command downloads each book's cover into `book_covers_additional/<isbn>.jpg` and repoints its `local_cover_path` in the DB, so the pages link to local files. **`generate` now runs the same cache pass automatically** (best-effort, quiet) before building: it is a no-op once every cover is cached (all books skipped → no network), bails out fast with a note if the host is unreachable (so `generate` never hangs), and can be skipped with `generate --no-cache`. Books that already have a curated local cover are skipped unless `--force`. Shared helpers `_download_cover()` / `_cache_covers()` back both the command and `generate`. The GUI's **Save + Regenerate** inherits the auto-cache via the no-arg `cmd_generate()`. See §13.

2. **Lead-story blurb derived from book notes** — **Done.** `render_hero()` now builds the lead story's `<p class="story-body">` from the first 100 characters of the book's *rendered* `my_notes` (the same text shown on its per-book page), via a new `_book_excerpt(book, length=100)` helper (strips HTML tags, decodes entities, collapses whitespace, appends `…`). The `hero_body` DB field is **no longer rendered** for the lead (the field remains in the schema and is still set by the `hero` command/migration, but is inert on the site). This makes the front-page blurb a genuine excerpt of Arjun's own writing rather than separately-authored editorial copy. See §6 / §9.
...
- `hero_body` is likewise now **inert for display** — still stored/seeded, but the lead blurb is generated from `my_notes` (see v2.5 item 2).
- As covers get cached locally (v2.5 item 1), more books carry a `local_cover_path` and the live site depends less on Open Library at view time.
...
| `book_covers_additional/` | Local cover images — curated covers **and** Open Library covers cached by `cache-covers` (v2.5) | assets |
...
| `ai_notes` | TEXT | "About" blurb (AI‑generated). **Inert** — not rendered anywhere on the site (the front‑page card grid that used to show it was removed in v2.2). Kept in the DB for reference only. |
...
| `hero_body` | TEXT | Formerly the lead‑story paragraph. **No longer rendered** (v2.5) — the lead blurb is now generated from the first 100 chars of the book's `my_notes` via `_book_excerpt()`. Still stored/seeded but inert. |
| `hero_progress` | TEXT | Progress chip (e.g. "Mid‑way") |
| `local_cover_path` | TEXT | Path under `book_covers_additional/`. Set manually for curated covers, and **automatically by `cache-covers`** when a book's Open Library cover is downloaded locally (v2.5). Takes priority over the ISBN URL. |
...
python librarian.py generate         # rebuild all generated files (auto-caches new covers first; --no-cache to skip)
```

### `cmd_generate()` — the build pipeline

**v2.5 — cover pre-cache step:** before producing any output, `cmd_generate()` runs a best-effort `_cache_covers(conn, fail_fast=True, quiet=True)` pass (unless `--no-cache` is passed). This downloads any not-yet-cached Open Library covers into `book_covers_additional/` and repoints `local_cover_path`, so the freshly-built pages link to local files. It does nothing once all covers are cached, and stops at the first unreachable result (printing a one-line note) so `generate` never hangs during an Open Library outage. See §13.

The generator then produces **five sets of outputs** in a single run — `index.html`, `library.html`, `books/[slug].html`, `essays/` (`_generate_essays`, see §12), and `library.md`:
...
- **`render_book_card(book)`** — formerly the expandable card for `index.html`. **Deleted** (v2.4) — the per-section grid it served was removed in v2.2.
- **`render_notes_preview(text, max_chars=150)`** — **NOT implemented** (roadmap; see §0).
- **`render_hero(conn)`** — newspaper hero for `index.html` (lead/side/bottom slots, dynamic volume count). **v2.5:** the lead slot's `story-body` is now produced by `_book_excerpt(b)` (from the book's notes), not the `hero_body` field.
- **`_book_excerpt(book, length=100)`** — **(v2.5)** returns the first `length` characters (plus `…`) of the book's per-book-page text: renders `my_notes` via `_notes_to_html`, strips tags, decodes entities, collapses whitespace. Returns `''` for books with no notes. Used for the lead hero blurb.
- **`_download_cover(isbn, timeout=20)`** — **(v2.5)** fetches one Open Library cover (`-L`, `?default=false`); returns `(data, None)` on success, `(None, 'missing')` for a 404/blank, or `(None, 'unreachable')` when the host is down/timed out.
- **`_cache_covers(conn, force=False, fail_fast=False, quiet=False)`** — **(v2.5)** shared core for `cache-covers` and `generate`: downloads missing covers, writes them to `book_covers_additional/<isbn>.jpg`, and updates `local_cover_path`. `fail_fast` stops at the first unreachable host; `quiet` suppresses per-book output; returns a counts dict. Skips books that already have a local cover unless `force`.
- **`e()`** — HTML escaping.
...
2. **Newspaper hero** (`%%NEWSPAPER_DYNAMIC%%`) — lead/side/bottom slots for currently-reading books. **v2.2:** each hero book's headline and cover now link to `books/[slug].html` via the page-turn animation. **v2.5:** the lead story's body paragraph is now the first 100 characters of the lead book's own notes (via `_book_excerpt`), not the `hero_body` field — a real excerpt of the per-book page rather than separate editorial copy.
...
- **Resolution order:** `local_cover_path` (under `book_covers_additional/`) first, then **Open Library** by ISBN (`https://covers.openlibrary.org/b/isbn/{ISBN}-M.jpg` for tiles, `-L` for per-book/detail pages).
...
### Local cover caching (v2.5) — offline-proof covers

Because covers historically loaded from `covers.openlibrary.org` *at view time*, an Open Library outage blanked every ISBN-based cover to the "No cover available" fallback all at once. The fix is to cache covers as local files so the site no longer depends on Open Library when viewed.

- **`python librarian.py cache-covers`** — for every book with an ISBN, downloads `…/{isbn}-L.jpg?default=false` into `book_covers_additional/<isbn>.jpg` and sets that book's `local_cover_path`. The `?default=false` makes Open Library return a real **404** for a missing cover instead of a blank 1×1 placeholder; a `<1000 bytes` guard catches any stray placeholder. Books that already have a (curated) local cover are skipped unless **`--force`**. Verbose per-book output.
- **Automatic on `generate`** — `cmd_generate()` runs the same pass (quiet, fail-fast) *before* building, so newly-added books' covers get cached and baked into the pages with no extra step. It does nothing once everything is cached (all books skipped → no network calls), and if the host is unreachable it bails after the first timeout with `Note: covers.openlibrary.org unreachable - skipped cover caching` rather than hanging. Skip with **`generate --no-cache`**.
- **Resilience:** once a cover is cached, it's a permanent local asset; subsequent Open Library outages can't blank it. During an outage, uncached books simply keep the "No cover available" fallback until the host returns and the next cache pass succeeds.
- **Shared implementation:** `_download_cover()` + `_cache_covers()` (see §6) back both the command and the `generate` auto-pass. Console output is ASCII-safe (`->`, `-`) to avoid `UnicodeEncodeError` on a Windows cp1252 console.
...
**Cache covers locally (recommended once, and after an Open Library recovery):**
1. `python librarian.py cache-covers` — downloads every book's cover into `book_covers_additional/` and repoints `local_cover_path`.
2. `python librarian.py generate` — bakes the local paths into the pages.
   (Day-to-day you can skip step 1 — `generate` auto-caches any new covers anyway.)
...
- Open Library and Google Fonts require network access; both degrade gracefully. **v2.5:** running `cache-covers` (or any `generate` while the host is up) pulls covers down as local files, so the live site stops depending on Open Library for already-cached books. The build itself also tolerates an Open Library outage — the auto-cache pass fails fast and `generate` completes regardless.
...
- Extend `test_librarian.py` to cover slug generation, `library.html`, `books/` output, essay generation, and the cover-cache helpers.

**Recently completed (v2.5):**
- ✅ Deleted dead `render_book_card()` from `librarian.py`.
- ✅ Local cover caching (`cache-covers` + `generate` auto-pass) to survive Open Library outages.
- ✅ Lead hero blurb derived from book notes.
